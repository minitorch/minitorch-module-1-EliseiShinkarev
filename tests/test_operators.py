...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    eps = 1e-4
    assert(sigmoid(a) >= 0.0, True)
    assert(sigmoid(a) <= 1.0, True)
    assert(1 - sigmoid(a), sigmoid(-1.0 * a))
    assert(sigmoid(0), 0.5)
    inc = sigmoid(a) < sigmoid(a + eps) if a > 0 else sigmoid(a) < sigmoid(a - eps)
    assert(inc, True)
...
def test_other(x: float) -> None:
    ...
    # raise NotImplementedError("Need to implement for Task 0.2")
    EPS = 1e-3
    assert_close(1, mul(x + EPS, inv(x + EPS)))
